...
namespace ChurvaInterpreter
{
	static class LexicalParser
	{
			...
			foreach (var line in source) {
				...
				if (initialIndent.Any()) {
					...
					if (newIndent != indent) {
						atoms.Add(
							new ParseAtom(
								linNum, 0, newIndent > indent ? ChurvaToken.INDENT : ChurvaToken.DEDENT, $"{newIndent}"
							)
						);
						indent = newIndent;
					}
					...
					indent = 0;
					atoms.Add(new ParseAtom(linNum, 0, ChurvaToken.DEDENT, $"{0}"));
                }
				...
				atoms.AddRange(
					TextCrawler(trimmedLine)
						.Select(
							sym => new ParseAtom(
								linNum, (ushort)sym.col, sym.isAlpha ? ChurvaToken.TEXT : ChurvaToken.OP, sym.text
							)
						)
				);
				atoms.Add(new ParseAtom(linNum, line.Length, ChurvaToken.NL, ""));
				++linNum;
			}
		}
		...
		{
			foreach (var atom in atoms.Where(a => a.Token == ChurvaToken.TEXT))
				if (atom.Text.All(ch => char.IsDigit(ch) || ch == '.'))
					atom.Token = ChurvaToken.NUMBER;
		}
		...
		private static void ReconstructFloats (IList<ParseAtom> atoms)
		{
			...
			for (int a = atoms.Count - 3; a >= 0; --a) {
				...
				if (inspect[0].Token == ChurvaToken.NUMBER && inspect[1].Text == "." &&
					inspect[2].Token == ChurvaToken.NUMBER) {
		...
		private static void ConstructLiterals (IList<ParseAtom> atoms)
		{
			for (int a = atoms.Count - 3; a >= 0; --a) {
				...
				if ((inspect[0].Text + inspect[2].Text == "''") || (inspect[0].Text + inspect[2].Text == "\"\"")) {
					inspect[0].Token = inspect[0].Text == "'" ? ChurvaToken.CHAR : ChurvaToken.STRING;
